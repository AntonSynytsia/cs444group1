...
\begin{enumerate}
    ...
    \end{enumerate}
    \item To evaluate and grade our work, the TA can follow the sections below, in chronological order, and replicate our Morse code LED trigger for their Raspberry Pi. If the TA already have the Linux Kernel installed, Raspbian Light setup, and the serial console operating, they can skip the \ref{setup} section and get right into the \ref{morse_compile} section for testing our Morse Code LED trigger on their own Raspberry Pi. If the TA is experienced, they can review each section for legitimacy without replicating and contact Group 1 for a demo of the Morse code LED trigger.
\end{enumerate}
