...
\begin{enumerate}
    ...
    \end{enumerate}
    \item To evaluate and grade our work, the TA can follow the sections below, in chronological order, and replicate our Morse code LED trigger for their Raspberry Pi. If the TA already have the Linux Kernel installed, Raspbian Light setup, and the serial console operating, they can get right into section \ref{morse_compile} for testing our Morse Code LED trigger on their own Raspberry Pi. If the TA is experienced, they can review each section for legitimacy without replicating and contact Group 1 for a demo of the Morse code LED trigger.
\end{enumerate}
...
\begin{enumerate}
\item Download \texttt{Image} from \texttt{arch/arm64/boot/Image} to your local file system, either using WinCP or another file transfer protocol. An easy way to do it is to first copy to base path and then download with WinCP to your computer:
...
To control Morse code speed, we utilize a device file and setup the show and store attributes for the device file. The device file is created with \texttt{device\_create\_file} function whenever the Morse trigger is activated and destroyed with the \texttt{device\_remove\_file} whenever the Morse trigger is deactivated. We use the \texttt{DEVICE\_ATTR} to register the \texttt{led\_speed\_show} and \texttt{led\_speed\_store} attributes.

A buffer received within the \texttt{led\_speed\_store} callback function is converted to an integer, clamped between 1 and 10000, and saved to our \texttt{morse\_trig\_data} structure instance for use within the \texttt{led\_morse\_function} timer function:
\inputminted[breaklines]{c}{cs7.c}

\texttt{led\_speed\_show}

Section \ref{run} describes how to control speed when the Morse code trigger is ran from Raspberry Pi.

The show message just prints out the space

\subsection{Repeat Control}
For the repeat control we used a device file and utilized the show and store attributes of the device file in order show and change if the message will repeat infinitely or it will only display once. The device file is created with \texttt{device\_create\_file} function whenever the Morse trigger is activated and destroyed with the \texttt{device\_remove\_file} whenever the Morse trigger is deactivated. We use the \texttt{DEVICE\_ATTR} to register the \texttt{led\_repeat\_show} and \texttt{led\_repeat\_store} attributes. \newline

A buffer is received from the device file and using the callback function \texttt{led\_repeat\_store} the buffer containing the Boolean for the repeat functionality is converted to an integer. For the repeat control, it is restricted to being 1 or 0, if the value is a 1 that means it will repeat the message infinitely, if the value is a 0 the message will only be repeated a single time.

Section \ref{run} describes how to control repeat mode when the Morse code trigger is ran from Raspberry Pi.

\subsection{Message Control}
For the message control we used a device file and utilized the show and store attributes of the device file in order to show and change the message that is currently being displayed on the raspberry pi.The device file is created with \texttt{device\_create\_file} function whenever the Morse trigger is activated and destroyed with the \texttt{device\_remove\_file} whenever the Morse trigger is deactivated. We use the \texttt{DEVICE\_ATTR} to register the \texttt{led\_message\_show} and \texttt{led\_message\_store} attributes. \newline
A buffer is received from the device file and using the callback function \texttt{led\_message\_store} the buffer containing the string is not converted because it is already in the correct variable type. For message control all ASCII characters are accepted, except the NULL terminator and new line characters, this is because they will end the message. Even though everything is accepted only digits and letters will be actually converted to Morse code, all other characters will be treated as the space character, they will also output an error to the console log. When a new message is set it will terminate the previous message after it has finished displaying the Morse code for the last character it was transmitting. The new message will then instantly start displaying on the raspberry pi.

Section \ref{run} describes how to assign message when the Morse code trigger is ran from Raspberry Pi.
...
\subsection{Running} \label{run}
Start a new serial console session, described in section \ref{rasp_setup}. Then run the following set of commands to activate and control the Morse code LED trigger:
\inputminted[breaklines]{bash}{cs5.sh}
The message can contain letters \texttt{A-Z}, either uppercase or lowercase, digits \texttt{0-9}, and spaces. All unrecognized characters are replaced with a space delay and are printed out to console.
...
\hline 76 &  & 2018-11-10 & Added version control to PDF & 2 & 105 & 0 \\
\hline 77 &  & 2018-11-10 & Added the finished writeup & 1 & 47 & 7 \\
\hline 78 &  & 2018-11-10 & Recompiled PDF for writeup 3 & 1 & 0 & 0 \\
\hline 79 &  & 2018-11-10 & Fixed typos & 1 & 10 & 10 \\
\hline 80 &  & 2018-11-10 & Recompiled writeup3 & 1 & 0 & 0 \\
\hline 81 &  & 2018-11-19 & add 2nd concurrency part. & 2 & 268 & 1 \\
\hline 82 &  & 2018-11-20 & Addded speeed & 1 & 18 & 12 \\
\hline 83 &  & 2018-11-22 & Added writeup setup for concurrency 3 and assignment 4 & 6 & 677 & 49 \\
\hline 84 &  & 2018-11-22 & Fixed makefile and instructions for Writeup3 & 3 & 2 & 2 \\
\hline 85 &  & 2018-11-22 & fixed writeup4 compiling & 2 & 54 & 0 \\
\hline 86 &  & 2018-11-22 & Work on char device & 1 & 99 & 94 \\
\hline 87 &  & 2018-11-22 & Fixining includes & 1 & 16 & 0 \\
\hline 88 &  & 2018-11-22 & Commenting out morse debug printk & 1 & 5 & 5 \\
\hline 89 &  & 2018-11-22 & Fixed speed option & 1 & 1 & 1 \\
\hline 90 &  & 2018-11-22 & Working on charachter device & 12 & 180 & 91 \\
\hline 91 &  & 2018-11-22 & Finished message device & 3 & 16 & 9 \\
\hline 92 &  & 2018-11-22 & Generated PDFs & 2 & 0 & 0 \\
\hline 93 &  & 2018-11-22 & Fixed readme & 1 & 1 & 1 \\
\hline 94 &  & 2018-11-22 & Unregistering char device for moment & 1 & 0 & 4 \\
\hline 95 &  & 2018-11-22 & Added options to control repeat or one-off modes & 3 & 132 & 85 \\
\hline 96 &  & 2018-11-22 & Fixed compile error & 1 & 48 & 6 \\
\hline 97 &  & 2018-11-22 & Fixed another error & 1 & 1 & 1 \\
\hline 98 &  & 2018-11-22 & Fixed one-off mode & 1 & 13 & 9 \\
\hline 99 &  & 2018-11-26 & fix bugs and add features & 1 & 33 & 14 \\
\end{longtable}
